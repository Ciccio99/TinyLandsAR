...
extern "C" void CreateSystemRenderingSurfaceMTL(UnityDisplaySurfaceMTL* surface)
{
    ...
    if ([surface->layer respondsToSelector: @selector(setDisplaySyncEnabled:)])
        [surface->layer performSelector: @selector(setDisplaySyncEnabled:) withObject: nil]; // BOOL arg, passing nil is NO, anything else YES

    CGFloat backgroundColorValues[] = {0, 0, 0, 1};
#if (UNITY_IOS && __IPHONE_9_0) || UNITY_OSX
    ...
    @synchronized(surface->layer)
    {
        ...
        OSAtomicCompareAndSwap32Barrier(surface->writeCount, surface->writeCount + 2, &surface->writeCount);
...
extern "C" void StartFrameRenderingMTL(UnityDisplaySurfaceMTL* surface)
{
    ...
    OSAtomicCompareAndSwap32Barrier(surface->bufferCompleted[surface->writeCount % 3], 0, &surface->bufferCompleted[surface->writeCount % 3]);
#endif
    surface->systemColorRB  = surface->drawableProxyRT[surface->writeCount % 3];
